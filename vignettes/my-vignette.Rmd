...
We repeat this function $B = 200$ times, and draw a histogram of the MLE of the $p$th coefficient. We pick $n = 1000$ and $p = 200$, and we set the variables to be from multivariate Gaussian from a AR(1) model with $\rho = 0.5$.
...
The goal of this package is to estimate the bias and standard deviation of the MLE more accurately than the classical theory. The procedure is based on the asymptotic distribution of the MLE when $n, p\to\infty$ at a fixed ratio $p / n\to \kappa \in (0, 1)$. The asymptotic theory is based on the distribution of the MLE when the variables $X$ is from a multivariate Gaussian. Now let's load the library. 
...
We will use the function `adjust_glm` to compute the adjusted MLE and standard error. Let's repeat the previous example and compute the MLE. Note here we specify `x = TRUE` and `y = TRUE` so the original data matrix is returned in `fit`. 
...
Now we call `adjust_glm`. There are three arguments for this function, but you only need to input the first one, the output `glm` object. Below, the second argument and third arguments are set at their default values, `verbose` specifies if progress should be printed, and `echo = TRUE` means that the input `glm` object is also returned in the output. `verbose = TRUE` by default because it helps to identify issues in the code and because it takes a while to run the function. 

```{r}
adjusted_fit <- adjust_glm(fit, verbose = FALSE, echo = TRUE)
```

We now show you how to access the estimated bias, and you can also find the adjusted MLE and their standard error for the last variable. You can compare the estimated bias `r adjusted_fit$param["alpha_s"]` with the observed bias `r mean(beta_hat[1, ]) / 2`. 
...
You can print out the coefficient tables by calling the function `summary` as usual.
