...
\]
The adjusted MLE is $\hat{\beta}^\mathrm{Adj} = \hat{\beta} / \alpha_star$ and adjusted std. is $\sigma_\star / \tau_j$. 
...
In this example, we can compute the true parameters and compare them with the estimates

```{r}
gamma <- sqrt(sum((R %*% beta)^2/n))
params <- find_param(kappa = p/n, gamma = gamma, beta0 = 0.5, verbose = FALSE)
params[1] # true alpha_star
params[3] # true sigma_star
```

You can compare also these estimates from observations from repeated samples. The observed average of $\hat{\beta}^\mathrm{MLE}_p$ is shown in black line, and its true value is shown in red line. 
